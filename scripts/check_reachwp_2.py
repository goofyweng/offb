...
class Server:
    ...
    def compute_stuff(self):
        if self.curposx is not None and self.targetwpx is not None:
            pub = rospy.Publisher('/uav2/check_waypoint_2', String, queue_size=10)
            pub_bool = rospy.Publisher('/uav2/check_waypoint_bool_2', Bool, queue_size=10)
            if abs(self.curposx - self.targetwpx) <  0.5 and abs(self.curposy - self.targetwpy) <  0.5 and abs(self.curposz - self.targetwpz) <  0.5:
                check_result = 'uav2 Reach waypoint!:', self.targetwpx, self.targetwpy, self.targetwpz % rospy.get_time()
                self.reachwp = True
                ...
                # print("Reach waypoint! ", self.targetwpx, self.targetwpy, self.targetwpz % rospy.get_time())
                 ...
                 check_result = 'uav2 Fly to next waypoint:', self.targetwpx, self.targetwpy, self.targetwpz % rospy.get_time()
                 self.reachwp = False
                 # rospy.loginfo("%s %r" %(check_result,self.reachwp))
                 pub.publish(check_result)
                 pub_bool.publish(self.reachwp)
...
if __name__ == '__main__':
    rospy.init_node('checker_2')

    server = Server()

    rospy.Subscriber("/uav2/mavros/local_position/pose", PoseStamped , server.curpos_callback)
    rospy.Subscriber("/uav2/mavros/setpoint_raw/local", PositionTarget, server.targetwp_callback)
